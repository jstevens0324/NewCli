...
class EmindersReminders extends AbstractPoller
{
    protected $pidfile = 'eminders-reminders.pid';

    protected function configure()
    {
        $this->setName('eminders:reminders')
             ->setDescription('Queues pending reminders for eMinders')
             ->setDefinition(array(
                new InputArgument(
                    'debug',
    ...
    public function execute(InputInterface $in, OutputInterface $out)
    {
        ...
        $sql = <<<SQL
            //SELECT cl.sendzaId                          AS sendzaId,
            /* em rem */
            SELECT COALESCE(cl.email, co.email, 'mailer@petwise.me') AS sendzaId,
                   r.rid                                AS reminderRid,
                   r.description                        AS reminderDescription,
                   r.dueDate                            AS reminderDueDate,

                   c.preferredContactTypeId,
            ...
            WHERE  ((
                     cl.remindersEnabled = 1
                   ) OR (
                     CURRENT_DATE() >= cl.reminderManualStart
                     AND CURRENT_DATE() <= DATE_ADD(cl.reminderManualStart, INTERVAL cl.reminderManualDays DAY)
                     AND cl.reminderManualDays > 0
                     AND cl.reminderManualStart IS NOT NULL
                   ))
...
SQL;
        ...
        //while(true) {
            ...
            foreach($result as $row) {
               ...
               /* $sql = "select rid from reminder where dsid = :dsid and patientRid = :patientRid and process_stat  > 0  and dueDate = :dueDate and description = :description ";
                $stmt = $conn->prepare($sql);
                $stmt->bindValue("dsid",$row['dsid']);                

                $stmt->bindValue("patientRid",$row['patientRid']);                
                $stmt->bindValue("dueDate",$row['reminderDueDate']);
                $stmt->bindValue("description",$row['reminderDescription']);
                $stmt->execute();
                ...
                $process_stat = 1;
                $conn->executeUpdate("update `reminder` SET process_stat = ".$process_stat." where `dsid` = ? AND `rid` = ?",array($row['dsid'],$row['reminderRid']));
                
                if($rowCount > 0
		  {
                    unset($result[$key]);
                    $msg_dup = "Duplicate: patient:".$row['patientRid']." ".$msg."\n";
                    $logger->writeLn($msg_dup);
                }*/
            ...
            if (!$debug) {
                //$service->queueEmindersBatch($result);
