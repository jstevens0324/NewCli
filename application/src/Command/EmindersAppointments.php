...
class EmindersAppointments extends AbstractPoller
{
    protected $pidfile = 'eminders-appointments.pid';

    protected function configure()
    {
        $this->setName('eminders:appointments')
             ->setDescription('Queues pending appointments for eMinders')
             ->setDefinition(array(
                new InputArgument(
                    'debug',
    ...
    public function execute(InputInterface $in, OutputInterface $out)
    {
        ...
        $sql = <<<SQL
            SELECT cl.sendzaId                          AS sendzaId,
                   ...
                   a.activeStartTime                    AS appointmentStartTime,
            ...
            WHERE  cl.autoAppointmentReminders = 1
                   ...
                   AND a.enabled = 1
                   AND a.confirmed = NULL
                   AND (((a.activeStartDate = CURRENT_DATE() AND a.activeStartTime > CURRENT_TIME()) OR (a.activeSTartDate > CURRENT_DATE())) 
		     AND (a.activeStartDate <= CURRENT_DATE() + INTERVAL cl.appointmentRemindersDaysInAdvance DAY))
		     AND (cml.id IS NULL OR cml.contactTypeId = IFNULL(c.preferredContactTypeId, {$contactTypeId}))
